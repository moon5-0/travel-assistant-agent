...
class LazyAgentRegistry:
    ...
    def __getitem__(self, agent_name: str):
        ...
        当外部代码使用 ``registry["information_query"]`` 时，Python 会自动
        调用本方法。已加载的 Agent 直接从缓存返回；首次访问时才动态导入
        对应 Skill 的 ``script/agent.py``，创建实例并写入缓存。
        """

        # 第二次及后续访问会命中缓存，不再重复导入和实例化。
        if agent_name in self.cache:
            return self.cache[agent_name]

        # 首次访问：把调度计划中的 Agent 名称解析为实际 Skill 目录名。
